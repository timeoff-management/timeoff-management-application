module "timeoff_service" {
  ...
  container_port        = 3000
  protocol              = "HTTP"
  alb_security_group_id = data.terraform_remote_state.global.outputs.alb_security_groups
...
module "timeoff_deploy" {
  ...
  application_name       = "AppECS-core-timeoff-http-3000-deploy"
