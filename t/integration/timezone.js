...
describe("Check Time zones", function() {
  let driver, user_email, today_usa, today_aus;
    ...
    open_page_func({
      ...
      .then(() =>
        driver.findElement(
          By.css(
            "table.month_" +
              moment(today_aus).format("MMMM") +
              " td.half_1st.day_" +
              moment(today_aus).format("D") +
              ".current_day_cell"
          )
        )
      )
    ...
    open_page_func({
      ...
      .then(month_caption => {
        expect(month_caption, "Ensure month is correct").to.be.eql(
          moment(today_aus).format("MMMM, YYYY")
        );
        done();
      });
  });
  ...
  it("Ensure that today_usa is one day behind the today_aus", function(done) {
    expect(moment(today_usa).format("YYYY-MM-DD")).to.be.not.eql(
      moment(today_aus).format("YYYY-MM-DD")
    );
    done();
  });
