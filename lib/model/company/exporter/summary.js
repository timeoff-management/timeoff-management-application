...
class CompanySummary {
  ...
  as_csv_data() {
    const self = this
    const results = []
    const date_format = self.company.get_default_date_format()
    ...
    self.users
      ...
      .sort(
        (a, b) =>
          sorter(
            departmentsMap[a.department_id].name,
            departmentsMap[b.department_id].name
      ...
      .forEach(u =>
        u.my_leaves.forEach(l => {
          ...
          results.push([
            departmentsMap[u.department_id].name,
            u.lastname,
            u.name,
            u.email,
            leaveTypesMap[l.leave_type_id].name,
            start_date,
            startDatePart,
            end_date,
            endDatePart
