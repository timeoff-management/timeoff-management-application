...
const schemaCreateNewLeave = Joi.object().required().keys({
...
function createNewLeave(args) {
  ...
  if (start_date.toDate() > end_date.toDate()) {
    Exception.throwUserError({
      ...
      system_error: `Failed to add new Leave for user ${employee.id} ``because start date ${start_date} happnned to be after end date ${end_date}`,
    });
  }
