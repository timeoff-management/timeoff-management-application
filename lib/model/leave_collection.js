...
function promise_to_group_leaves(leaves,holiday_year_start_month) {
  ...
  leaves.forEach(leave => {
    ...
    if(moment.utc(leave.get_start_leave_day().date).format('M') <= holiday_year_start_month){
      year--;
    }
  ...
  grouped_leaves.forEach(group => {
    group.total_deduction = _.reduce(
      group.leaves.map(leave => leave.get_deducted_days_number({holiday_year_start_month:holiday_year_start_month})),
