  ...
  flow = flow.then(users => Promise.map(
    ...
    user => {
      ...
      return Promise.resolve(user.getCompany().then(c => {
        ...
        .then(() => user.reload_with_leave_details({holiday_year_start_month :holidayMonthStart, year:moment.utc(yearFrom, 'YYYY')}))
        .then(user => user.promise_allowance({
          year: moment.utc(yearFrom, 'YYYY'),
         holiday_year_start_month : holidayMonthStart,
