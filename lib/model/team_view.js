    ...
    .forEach(node => {
      ...
      node
        ...
        .forEach( day => {
          ...
          // Capture statistics by Leave types
          if (leave_type) {
            if (day.is_leave_afternoon && day.is_leave_morning) {
              leave_type_stat[ leave_type.id ] ++;
            } else {
              leave_type_stat[ leave_type.id ] = leave_type_stat[ leave_type.id ] + 0.5;
            }
          }

          // Abort if current leave type does not use allowance
      ...
      let statistics = {
        ...
        leave_type_break_down : {
          ...
          pretty_version : leave_types
            ...
            .map(lt => ({ name : lt.name, stat : leave_type_stat[ lt.id ] })),
        }
      };
