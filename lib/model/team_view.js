...
TeamView.prototype.promise_team_view_details = function (args) {
  ...
  return promise_users_and_leaves.then(function (users_and_leaves) {
    ...
    var leave_month_summary_users = [];
    // Calculate leave summary for current month for each user
    users_and_leaves.forEach(function (user_and_leaves) {
      leave_month_summary_users.push(
        user_and_leaves.user.promise_my_leave_summary_for_month({month: base_date}).then(function (summary) {
          user_and_leaves.total_leave = summary.total_leave;
          delete summary['total_leave']; // Cos of forEach in hbs file we need to remove it
          user_and_leaves.leave_summary = summary;

        })
      );
    });

    return Promise.all(leave_month_summary_users).then(function () {
