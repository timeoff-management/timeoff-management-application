...
/**
 * This function adds the object leave_summary and total_leave for the given month to the user summary
 * @param userSummary Item from users_and_leaves
 * @param leave_summary This should be object with summary typesIDs nad counter initialized to 0
 * @param bank_holidays Bank holidays map so we can filter out the banking holidays.
 * @param schedule User schedule so we can filter only working days.
 * @param date It's expected that this will be moment date, it's used to filter the specific month
 */
function calculate_user_approved_leave_summary(userSummary, leave_summary, bank_holidays, schedule, date) {

  var total_leave = 0; // Total numbers of missing days.
  ...
  userSummary.leave_days.forEach(function (leave_day) {
    ...
    if (leave_day.leave.is_approved_leave() && schedule.is_it_working_day({day: moment(leave_day.date)})
      ...
      // TODO: Fix this to Leave.leave_day_part_all instead of just using === 1
      if (leave_day.day_part === 1) {
        leave_summary[leave_day.leave.leaveTypeId].count += 1;
        total_leave += 1;
      } else {
        leave_summary[leave_day.leave.leaveTypeId].count += 0.5;
        total_leave += 0.5;
      }
    }
  });
  ...
  userSummary.total_leave = total_leave;
}
...
TeamView.prototype.promise_team_view_details = function(args){
  ...
  return user.get_company_for_export().then(function (company) {
    ...
    company.leave_types.forEach(function (leave_type) {
      leave_summary[leave_type.id] = {name: leave_type.name, count: 0};
    });
    ...
    return promise_users_and_leaves.then(function (users_and_leaves) {
      ...
      users_and_leaves.forEach(function (item) {
        calculate_user_approved_leave_summary(item, JSON.parse(JSON.stringify(leave_summary)), bank_holiday_map, schedule, base_date);
      });
