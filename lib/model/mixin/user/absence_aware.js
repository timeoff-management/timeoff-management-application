...
module.exports = function(sequelize) {
  ...
  this.promise_my_leaves = function(args) {
    ...
    const where_clause = {
      status: args && args.filter_status,
      [Op.or]: args &&
      !args.ignore_year && {
        date_start: {
          [Op.between]: [
    ...
    var promise_my_leaves = this.getMy_leaves({
      ...
      .then(function(leaves) {
        ...
        return Promise.resolve(leaves).map(
          function(leave) {
            return leave.promise_approver().then(function(approver) {
  ...
  this.calculate_number_of_days_taken_from_allowance = function(args) {
    ...
    leaves_to_traverse.forEach(function(leave) {
      leave.user.cached_schedule = self.cached_schedule;
    });

    // If leave_type was provided, we care only about leaves of that type
    if (leave_type) {
      leaves_to_traverse = _.filter(leaves_to_traverse, function(leave) {
        return leave.leaveTypeId === leave_type.id;
      });
    }
    ...
    return (
      _.reduce(
        _.map(
          _.filter(leaves_to_traverse, function(leave) {
  ...
  (this.get_leave_statistics_by_types = function(args) {
    ...
    var statistic_arr = _.map(_.pairs(statistics), function(pair) {
      return pair[1];
    });

    statistic_arr = _.sortBy(statistic_arr, "days_taken").reverse();

    if (limit_by_top) {
      statistic_arr = _.first(statistic_arr, 4);
    }

    return _.sortBy(statistic_arr, function(rec) {
      return rec.leave_type.name;
    });
  }),
