...
module.exports = function(sequelize){
  ...
  this._get_calendar_months_to_show = function(args){
    ...
    if (show_full_year) {
      return _.map(orderMonths, function(i){
        var calYear = moment.utc(year).month() > starting_month ? moment.utc(year).add(1,'year').format('YYYY'):moment.utc(year).format('YYYY');
        ...
        return this_user.getMy_leaves({
          where : {
            ...
            $or : {
              date_start : {
                $between : [
                  moment.utc(year).subtract(moment.utc(year).month() > holiday_year_start_month?0:1, "year").month(args.holiday_year_start_month ).startOf('month').format('YYYY-MM-DD'),
                  moment.utc(year).add(moment.utc(year).month() > holiday_year_start_month?1:0, "year").month(args.holiday_year_start_month -1).endOf('month').format('YYYY-MM-DD'),
                ]
              },
              date_end : {
                $between : [
  ...
  this.promise_my_leaves = function(args){
    ...
    if(year.month() < args.holiday_year_start_month){
      year =   moment.utc(year).subtract(1, "year");
    }
    ...
    if (args && ! args.ignore_year) {
      where_clause['$or'] = {
        date_start : {
          $between : [
            moment.utc(year).month(args.holiday_year_start_month ).startOf('month').format('YYYY-MM-DD'),
            moment.utc(year).add(1, "year").month(args.holiday_year_start_month -1).endOf('month').format('YYYY-MM-DD HH:mm'),
          ]
        },
        date_end : {
          $between : [
  ...
  this.promise_adjustment_and_carry_over_for_year = function(holiday_year_start_month,year){
    let self = this;
    ...
    return self
      .getAdjustments({
        where : { year :  month >  holiday_year_start_month ?  year : year -1 }
      })
  ...
  this.promise_adjustmet_for_year = function(holiday_year_start_month ,year){
    let self = this;
  ...
  this.promise_to_update_adjustment = function(args) {
    ...
    return sequelize.models.UserAllowanceAdjustment
      .findOrCreate({
        where : {
          user_id : self.id,
          year    : month>  holiday_year_start_month ?  year : year -1,
        },
