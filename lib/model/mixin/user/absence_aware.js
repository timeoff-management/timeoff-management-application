...
module.exports = function(sequelize){
    ...
    return this.getMy_leaves({
      where : {
        ...
        $or : {
          date_start : {
            $between : [
              moment(date).startOf(date_type).format('YYYY-MM-DD'),
              moment(date).endOf(date_type).format('YYYY-MM-DD'),
            ]
          },
          date_end : {
            $between : [
  ...
  this.promise_my_leave_summary_for_month = function(args){
    var month = args.month || moment().format('YYYY-MM'), // The month also has to have a year
        me = this;

    // We need to populate user (this) with company and schedule

    // We can't user user.reload_with_session_details because this function
    // doesn't populate company with bank_holidays ...
    return me.get_company_with_all_leave_types_and_bank_holidays().then(function (company) {
      me.company = company;

      return me.reload_with_leave_details({month});
    }).then(function () {
      return me.promise_my_leaves_for_calendar({month});
    }).then(function (leaves) {
      var approved_leaves = leaves.filter(function (leave) {
        return leave.is_approved_leave();
      });

      var summary = {};
      summary['total_leave'] = 0;
      // Prepare the summary object with the names of leave types and ids
      me.company.leave_types.forEach(function(leave_type){
        summary[leave_type.dataValues.id] = {
          name: leave_type.dataValues.name,
          count: 0
        }
      });

      // Calculate the number of leave days for each leave in given month
      approved_leaves.forEach(function (leave) {

        var deductedDays = leave.get_deducted_days_number(
        {
          user: me,
          year: month,
          month: month,
          ignore_allowance: true //important to filter bank holidays and weekends
        });

        summary['total_leave'] += deductedDays;
        summary[leave.leaveTypeId].count += deductedDays;
