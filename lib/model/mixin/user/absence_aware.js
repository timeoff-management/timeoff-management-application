...
module.exports = function(sequelize){
  ...
  this.promise_my_leaves = function(args){
    ...
    .then(leaves => promise_to_sort_leaves(leaves));
  ...
  this.promise_total_number_of_days_in_allowance = function(year){
    ...
    return Promise
      ...
      // Fetch Adjustment for current year
      .then(() => self.promise_adjustmet_for_year(year))

      .then(adjustment => Promise.resolve(
  ...
  this.promise_to_update_adjustment = function(args) {
    let
      self = this,
      year = args.year || moment().format('YYYY'),
      adjustment = args.adjustment;
