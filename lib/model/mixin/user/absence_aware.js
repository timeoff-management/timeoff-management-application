...
module.exports = function(sequelize){
  ...
  this.promise_calendar = function(args) {
    var
      ...
      holiday_start_month          = args.holiday_start_month || 0,
    ...
    var months_to_show = this_user._get_calendar_months_to_show({
      ...
      starting_month : holiday_start_month
    });
        ...
        return this_user.getMy_leaves({
          where : {
            ...
            $or : {
              date_start : {
                $between : [
                  moment.utc(year).subtract(1, "year").month(args.holiday_start_month).startOf('month').format('YYYY-MM-DD'),
                  moment.utc(year).month(args.holiday_start_month-1).endOf('month').format('YYYY-MM-DD HH:mm'),
                ]
              },
              date_end : {
                $between : [
  ...
  this.promise_my_active_leaves = function(args) {
    ...
    var holiday_start_month = args.holiday_start_month;

    return this.promise_my_leaves({
      year          : year,
      holiday_start_month          : holiday_start_month,
      filter_status : [
  ...
  this.promise_adjustmet_for_year = function(holiday_start_month,year){
    let self = this;

    return self
      .promise_adjustment_and_carry_over_for_year(holiday_start_month,year)
      .then(combined_record => Promise.resolve(combined_record.adjustment));
  };

  this.promise_carried_over_allowance_for_year = function(holiday_start_month,year){
    let self = this;
      ...
      where : {
        ...
        $or : {
          date_start : {
            $between : [
              moment.utc(year).subtract(1, "year").month(args.holiday_start_month).startOf('month').format('YYYY-MM-DD'),
              moment.utc(year).month(args.holiday_start_month).endOf('month').format('YYYY-MM-DD HH:mm'),
            ]
          },
          date_end : {
            $between : [
  ...
  this.validate_leave_fits_into_remaining_allowance = function(args){
    ...
    holiday_start_month      = args.holiday_start_month ;

    // Make sure object contain all necessary data for that check
    return self.reload_with_leave_details({
      holiday_start_month: self.company.holiday_year_start_month,
      year : year.clone(),
    })
    ...
    .then(employee =>
      employee.promise_allowance({year,holiday_start_month})
