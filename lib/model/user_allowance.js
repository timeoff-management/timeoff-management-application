...
class UserAllowance {
  ...
  get employement_range_adjustment() {
    ...
    var end = moment.utc(now).add(moment.utc(now).month() >  self.user.company.holiday_year_start_month?1:0, "year").month( self.user.company.holiday_year_start_month-1).endOf('month').format('YYYY-MM-DD');

   let end_date = self.user.end_date && moment.utc(self.user.end_date).year() <= now.year()
