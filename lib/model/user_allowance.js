...
const
  ...
  schema_promise_allowance = Joi.object().required().keys({
    ...
    user : schema_user,
    now  : schema_now,
    holiday_start_month: Joi.number().default(-1),
    /**
...
class UserAllowance {
  ...
  static promise_allowance(args) {
    ...
    if(holiday_start_month === -1){
      holiday_start_month = user.company.holiday_year_start_month;
    }
    ...
    if ( user.my_leaves === undefined ) {
      flow = flow.then(() => user.reload_with_leave_details({holiday_start_month,year}));
    }

    // Fetch adjustment and Carry over allowance
    flow = flow.then(() => user.promise_adjustment_and_carry_over_for_year(holiday_start_month,year));
    ...
    flow = flow.then(() => Promise.resolve(
      number_of_days_taken_from_allowance = user.calculate_number_of_days_taken_from_allowance({holiday_start_month:holiday_start_month,year : year.format('YYYY')})
    ));
