    ...
    return (
      sequelize.models.User.findAll({
        ...
        .then((company_definition) => {
          const company_json = _.omit(dump_json, (value, key, object) => !(company_definition.hasOwnProperty(key) && key !== 'id'))

          return sequelize.models.Company.create(company_json).then((
            company
          ) => {
        ...
        // Insert departments
        //
        .then(() => promise_to_restore_departments({
          dump_json,
          id_maps,
          model: sequelize.models
        }))
        ...
        //
        .then(() => Promise.all(
          _.map(_.values(id_maps.department), (department_id) => sequelize.models.Department.find({
            where: { id: department_id }
          }))
        ).then((departments_to_update) => Promise.all(
        ...
        // insert leaves
        .then(() => promise_to_restore_leaves({
          dump_json,
          id_maps,
          model: sequelize.models
        }))
