...
get_deducted_days : function(args) {
  ...
  if (args && args.hasOwnProperty('holiday_year_start_month')) {
   holiday_year_start_month  = moment.utc(args.holiday_year_start_month , 'MM');
  }
  ...
  leave_days = _.filter(
    _.map(this.get_days(), function(leave_day){
      ...
      if (year &&holiday_year_start_month ){
        if(moment.utc(leave_day.date).year() === year.year()-1 && (moment.utc(leave_day.date).month()+1) < (holiday_year_start_month .month()+1 )){
          return;
        }

        if(moment.utc(leave_day.date).year() === year.year()  && (moment.utc(leave_day.date).month()+1) < (holiday_year_start_month .month()+1)){
     //     return;        
     console.log(  moment.utc(leave_day.date).month()+1 )
        console.log( holiday_year_start_month .month()+1)
        }
      }
