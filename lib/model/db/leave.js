  ...
  leave_days = _.filter(
    _.map(this.get_days(), function(leave_day){
      ...
      if (year &&holiday_year_start_month ){
        if(moment.utc(leave_day.date).year() <= year.year()-1 &&moment.utc(leave_day.date).month() <= holiday_year_start_month .month() ){
          return;
        }
        if(moment.utc(leave_day.date).year() === year.year()  && moment.utc(leave_day.date).month() >holiday_year_start_month .month()){
          return;
        }
      }
