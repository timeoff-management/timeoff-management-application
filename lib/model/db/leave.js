...
get_deducted_days : function(args) {
  ...
  if (args && args.hasOwnProperty('holiday_start_month')) {
    holiday_start_month = moment.utc(args.holiday_start_month, 'MM');
  }
  ...
  leave_days = _.filter(
    _.map(this.get_days(), function(leave_day){
      ...
      if (year && holiday_start_month){
        if(moment.utc(leave_day.date).year() <= year.year()-1 &&moment.utc(leave_day.date).month() <=  holiday_start_month.month() ){
          return;
        }
        if(moment.utc(leave_day.date).year() === year.year()  && moment.utc(leave_day.date).month() >holiday_start_month.month()){
          return;
        }
      }
