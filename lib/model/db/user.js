...
function get_instance_methods(sequelize) {
  ...
  return {
    ...
    promise_users_I_can_manage : function(){
      ...
      return this_user.promise_supervised_department()
     ...
     *
     * */
    promise_supervisers : function(){
      return this.getDepartment({
        scope : ['with_boss', 'with_supervisors'],
      })
      .then( department => Promise.resolve( Array.concat( [department.boss], department.supervisers ) ) );
    },

    promise_supervised_department : function() {
      let self = this;

      return sequelize.models.DepartmentSupervisor.findAll({ where : { user_id : self.id } })
        // Obtain departments current user supervises as secondary superviser
        .then(department_supervisers => department_supervisers.map( obj => obj.department_id ))
        .then( department_ids => {
    ...
    remove : function() {
      ...
      return self.promise_supervised_department()
        .then(departments => {
