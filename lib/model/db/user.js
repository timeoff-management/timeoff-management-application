...
module.exports = function(sequelize, DataTypes) {
  ...
  function get_instance_methods(sequelize) {
    return {
      ...
      promise_supervised_users: function() {
        ...
        return self.promise_supervised_departments().then(departments => self.sequelize.models.User.findAll({
      ...
      reload_with_session_details: function() {
        const self = this
        console.log('Reload with session details...')
        return Promise.join(
        ...
        return (
          self
            ...
            .then((leaves) =>
              // remove all leaves
              Promise.all(
                _.map(leaves, (leave) => leave.destroy())
              )
            )
  ...
  function get_class_methods(sequelize) {
    return {
      ...
      get_user_by_reset_password_token: function(token) {
        const self = this
        const unpacked_token = new Buffer(token, 'base64').toString('ascii')
  ...
  function withScopes() {
    this.loadScope = function(models) {
      models.User.addScope('active', () => ({ where: models.User.get_active_user_filter() }))
