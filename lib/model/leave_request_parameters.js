...
console.log('>>> same day: ' + moment(leave_day.date).format('YYYY-MM-DD') + ' ' + moment(this[point_name]).format('YYYY-MM-DD'));
console.log('>>>> day part '+leave_day.day_part);
console.log(' not a all day '+ this[point_name+'_part'] +' '+ model.LeaveDay.leave_day_part_all() );
console.log(' 4th line ' + leave_day.day_part + ' ' + this[point_name+'_part']  );

    var return_val = false;
console.log('RETURN: '+( return_val ? 'TRUE' : 'FALSE' ));
console.log('4th line eval: '+( (leave_day.day_part !== this[point_name+'_part']) ? 'TRUE' : 'FALSE' ));
    ...
    if (
        ...
        (! leave_day.is_all_day_leave())
          &&
        (String(this[point_name+'_part']) !== String(model.LeaveDay.leave_day_part_all()))
          &&
