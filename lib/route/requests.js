...
function leave_request_action(args) {
  ...
  return function(req, res) {
    ...
    Promise.try(() => req.user.promise_leaves_to_be_processed())
      .then((leaves) => {
        const leave_to_process = _.find(leaves, (leave) => (
...
router.post('/cancel/', (req, res) => {
  ...
  Promise.try(() => req.user.promise_cancelable_leaves())
    .then((leaves) => {
      const leave_to_cancel = _.find(leaves, (leave) => String(leave.id) === String(request_id))
    ...
    .then((leave) => {
      ...
      return Slack.promise_leave_request_cancel_slacks({
        leave
      }).then(() => Promise.resolve(leave))
    })
    .then((leave) => {
