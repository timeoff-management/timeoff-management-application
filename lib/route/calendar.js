...
router.get('/wallchart/', function(req, res){
  ...
  // Promise departmens either supervised by current user or one that she belongs to
  var promise_departments = Promise.join(
    ...
    function(my_department, supervised_departments){
      ...
      return Promise.resolve(supervised_departments);
    }
  );

  // Calculate users and leaves for every departmet
  ...
  Promise.resolve(promise_users_and_leaves)
