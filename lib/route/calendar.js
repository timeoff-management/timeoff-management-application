...
router.get('/', function(req, res) {
  ...
  var show_full_year = validator.toBoolean(req.param('show_full_year')),
    show_full_leave_type_statistic = validator.toBoolean(req.param('show_full_leave_type_statistic'));
  ...
  Promise.join(
    ...
    function(calendar, company, user){
      res.render('calendar', {
        ...
        show_full_year : show_full_year,
        leave_type_statistics : user.get_leave_statistics_by_types({
          limit_by_top : ! show_full_leave_type_statistic,
        }),
        show_full_leave_type_statistic : show_full_leave_type_statistic,
...
function promise_feed_of_type(args) {
  ...
  if (! feed) {
    feed_promise = user.Model().UserFeed.promise_new_feed({
      user : user,
      type : type,
    });
  } else {
