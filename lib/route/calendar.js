...
router.post('/bookleave/', function(req, res){
    ...
    Promise.join (
        ...
        function(users, company, valide_attributes){
            ...
            // TODO Following lines is better to mmove somewhere into its own 
            // method so it is reusable
            return employee.promise_superviser()

            .then(function(superviser){
                return model.Leave.create({
                    UserId           : employee.id,
                ...
                .then(function(leave){
                    var leave_request_attributes = _.clone( valide_attributes );
                    leave_request_attributes.leave = leave;

                    return model.LeaveDay.bulkCreate(
                        model.LeaveDay.get_objects_for_bulk_create(
                            leave_request_attributes
                        )
                    );

                    // TODO check that days records were added, if no, remove leave
                    // record as it looks like it sits withing week end
                });
            });
        }
    )
