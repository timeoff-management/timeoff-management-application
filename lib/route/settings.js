...
router.post("/company/", function (req, res) {
  const name = validator.trim(req.body.name),
    ...
    share_all_absences =
      validator.toBoolean(req.body.share_all_absences) || false,
    isTeamViewHidden =
      validator.toBoolean(req.body.is_team_view_hidden) || false;
  ...
  req.user
    ...
    .then(function (company) {
      ...
      if (!bank_holiday_to_remove) {
        ...
        req.session.flash_error(
          "Cannot remove bank holiday: wronge parameters"
        );
  ...
  req.user
    ...
    .then(function (company) {
      ...
      return Promise.all([
        ...
        _.map(company.leave_types, function (leave_type) {
...
router.post("/company/integration-api/", (req, res) => {
  const featureIsEnabled = validator.toBoolean(
    req.body.integration_api_enabled
  );
