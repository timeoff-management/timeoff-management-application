        ...
        return Promise.all([
            _.map(
                ...
                function(department, index){
                    ...
                    if (!validator.matches(name, /^[a-z0-9 \.\,]+$/i)){
                        req.session.flash_error(
                            'New name of '+department.name+' should contain only letters and numbers'
                        );
                    }
                    // New allowance should be from range of (0;50]
                    if (!validator.isFloat(allowence)) {
                        req.session.flash_error(
                            'New allowence for '+department.name+' should be numeric'
                        );
                    } else if (!((0 < allowence) && (allowence <= 50))) {
                        req.session.flash_error(
                            'New allowence for '+department.name+' should be between 0.5 and 50 days'   
                        );
                    }
                    ...
                    if (!validator.isNumeric( boss_id ) ) {
                        req.session.flash_error(
                            'New boss reference for '+department.name+' should be numeric'
                        );
                    } else if (_.contains(
                        _.map(
                        ...
                        req.session.flash_error(
                            'New boss for '+department.name+' is unknown'
                        );
                    }
                    ...
                    department.allowence               = allowence;
                    department.include_public_holidays = include_public_holidays;
                    department.bossId                  = boss_id;
