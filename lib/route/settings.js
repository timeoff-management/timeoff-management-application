...
router.post('/bankholidays/', function(req,res){
    var name = validator.trim(req.body['name__new']),
        date = validator.trim(req.body['date__new']),
        model= req.app.get('db_model');
...
function get_and_validate_leave_type(args) {
  ...
  if ( ! validator.matches(color, /^leave_type_color_\d+$/)) {
    ...
    if (typeof limit !== 'number' && !validator.isNumeric(limit)) {
...
function get_and_validate_ldap_auth_configuration(args) {
  ...
  //
    var url = validator.trim(params.url),
        ...
        ldap_auth_enabled = params.ldap_auth_enabled && validator.toBoolean(params.ldap_auth_enabled),
