...
router.post("/company/", function(req, res) {
  const name = req.body.name && validator.trim(req.body.name),
    ...
    carriedOverDays = req.body.carry_over && validator.trim(req.body.carry_over),
    share_all_absences =
  ...
  req.user
    ...
    .then(company => {
      ...
      company.share_all_absences = share_all_absences || false;
...
function get_and_validate_ldap_auth_configuration(args) {
  ...
  var url = validator.trim(req.body["url"]),
    ...
    allow_unauthorized_cert = validator.toBoolean(
      req.body["allow_unauthorized_cert"]
    ),
