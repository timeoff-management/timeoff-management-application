...
router.post('/company/', function(req, res){
    ...
    var name = validator.trim(req.body.name),
        ...
        share_all_absences = (req.body.share_all_absences && validator.toBoolean(
...
router.post('/schedule/', function(req, res){
  ...
  req.user.getCompany()
    ...
    // Action failed
    .catch(function(error){
        console.error('An error occurred while saving schedule: ' + error, error.stack);
...
router.post('/bankholidays/import/', (req, res) => {
  ...
  Promise
    ...
    .catch(error => {
      ...
      console.log(
          'An error occurred when trying to import default bank holidays by user ' + req.user.id, error, error.stack
      );
      console.dir(error);
...
router.post('/bankholidays/delete/:bank_holiday_number/', function(req, res){
    ...
    if (typeof bank_holiday_number !== 'number' && (!bank_holiday_number || !validator.isInt(bank_holiday_number))) {
        console.error(
...
router.post('/leavetypes/delete/:leave_type_id/', function(req, res){
    ...
    if (typeof leave_type_id !== 'number' && (!leave_type_id || !validator.isInt(leave_type_id))) {
        console.error(
...
function get_and_validate_bank_holiday(args) {
  ...
  // Get user parameters
    var name = validator.trim(params['name__' + index]),
        date = validator.trim(params['date__' + index]);
...
function get_and_validate_leave_type(args) {
  ...
  // Get user parameters
  let
      name = params['name__' + suffix] && validator.trim(params['name__' + suffix]),
      color = (params['color__' + suffix] && validator.trim(params['color__' + suffix])) || 'leave_type_color_1',
      limit = (params['limit__' + suffix] && validator.trim(params['limit__' + suffix])) || 0,
      first_record = (params['first_record'] && validator.trim(params['first_record'])) || 0,
      use_allowance = (params['use_allowance__' + suffix] && validator.toBoolean(
          params['use_allowance__' + suffix]
      )) || false;
...
function get_and_validate_ldap_auth_configuration(args) {
  ...
  //
    var url = validator.trim(params.url),
        binddn = validator.trim(params.binddn),
        bindcredentials = validator.trim(params.bindcredentials),
        searchbase = validator.trim(params.searchbase),
        ldap_auth_enabled = (params.ldap_auth_enabled && validator.toBoolean(params.ldap_auth_enabled)) || false,

  // Fetch the password of current user that is valid in LDAP system
        password_to_check = validator.trim(params.password_to_check);
