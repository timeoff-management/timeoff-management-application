...
router.post('/company/', (req, res) => {
  ...
  const date_format = req.body.date_format && validator.trim(req.body.date_format)
...
router.post('/company/authentication/', (req, res) => {
  req.user
    ...
    .then((company) => {
      ...
      const auth_func = function(email, password) {
...
function get_and_validate_leave_type(args) {
  ...
  const use_allowance = validator.toBoolean(req.body['use_allowance__' + suffix])
  ...
  const throw_user_error = function(message) {
...
router.get('/company/backup/', (req, res) => {
  ...
  req.user
    ...
    .then(company =>
      companyExporter.promiseCompanySummary({ company })
    )
