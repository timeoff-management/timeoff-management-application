...
router.post("/company/", function(req, res) {
  const name = validator.trim(req.param("name")),
    ...
    share_all_absences = validator.toBoolean(req.param("share_all_absences"));
...
function get_and_validate_leave_type(args) {
  ...
  // Get user parameters
  let name = validator.trim(req.param("name__" + suffix)),
    color =
      validator.trim(req.param("color__" + suffix)) || "leave_type_color_1",
