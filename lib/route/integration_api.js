  ...
  router.get(
    ...
    (req,res) => {
      ...
      let result = req.user
        ...
        .then(company => Promise.resolve(company.users) );

      if (currentDeparmentId) {
        result = result.then(users => users.filter(u => u.DepartmentId === currentDeparmentId));
      }
      ...
      result = result.then(report => report.map(
        ({user:{id, email, name, lastname}, leaves}) =>
        ({ user:{id, email,name,lastname}, leaves : leaves.map(l => ({ id: l.id }))  })
      ));

      result = result.then(data => res.json(data));
