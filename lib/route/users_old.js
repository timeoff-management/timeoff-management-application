...
function promise_user_list_data_for_rendering(args) {
  ...
  const users_info_for_rendering = users_info.map(ui => ({
...
function get_and_validate_user_parameters(args) {
  ...
  const lastname = req.body.lastname && validator.trim(req.body.lastname)
  let email = req.body.email_address && validator.trim(req.body.email_address)
  const department_id =
    req.body.department && validator.trim(req.body.department)
  let start_date = req.body.start_date && validator.trim(req.body.start_date)
  let end_date = req.body.end_date && validator.trim(req.body.end_date)
  const adjustment = req.body.adjustment && validator.trim(req.body.adjustment)
  const password =
    req.body.password_one && validator.trim(req.body.password_one)
  const password_confirm =
    req.body.password_confirm && validator.trim(req.body.password_confirm)
  const admin = (req.body.admin && validator.toBoolean(req.body.admin)) || false
  const auto_approve =
    (req.body.auto_approve && validator.toBoolean(req.body.auto_approve)) ||
    false
