...
var ensure_we_are_not_removing_last_admin = function(args) {
  ...
  if (
    ...
    return model.User.count({
      ...
      req.session.flash_error(
        "This is last admin witihn company. Cannot revoke admin rights."
      );
      throw new Error(
...
router.post("/edit/:user_id/", function(req, res) {
  ...
  Promise.try(function() {
    ...
    .then(function() {
      ...
      console.log("GOOD TO GO", new_user_attributes);
...
function get_and_validate_user_parameters(args) {
  ...
  var name = req.body["name"] && validator.trim(req.body["name"]),
    lastname = req.body["lastname"] && validator.trim(req.body["lastname"]),
    email = req.body["email_address"] && validator.trim(req.body["email_address"]),
    department_id = req.body["department"] && validator.trim(req.body["department"]),
    start_date = req.body["start_date"] && validator.trim(req.body["start_date"]),
    end_date = req.body["end_date"] && validator.trim(req.body["end_date"]),
    adjustment = req.body["adjustment"] && validator.trim(req.body["adjustment"]),
    password = req.body["password_one"] && validator.trim(req.body["password_one"]),
    password_confirm = req.body["password_confirm"] && validator.trim(req.body["password_confirm"]),
