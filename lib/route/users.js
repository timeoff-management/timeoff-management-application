...
router.post('/import/', function(req, res){
  ...
  parseAsync
    ...
    .then(parsed_data => {
      ...
      // TODO consider limiting number of users

      // Ditch header
      parsed_data.shift()

      return Promise
        .map(parsed_data, user_vector => {
          let attributes = {
            email        : user_vector[0],
            lastname     : user_vector[1],
            name         : user_vector[2],
            password : uuid.v4(),
            companyId : req.user.companyId,
            DepartmentId : XXXXXXX,
          };

          return model.User.create(attributes)
        })
        .then(new_users => {
          // NOTE  consider sending emails here
          return Promise.resolve(new_users);
        });
    })
    .then(function(company){
      req.session.flash_message('Successfully imported users');
      res.redirect_with_session('/users/import/');
    })
    .catch(function(error){
      req.session.flash_error('Failed to import users, reason: '+error);
      res.redirect_with_session('/users/import/');
    });
});
