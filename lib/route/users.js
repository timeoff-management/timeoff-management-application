...
function get_and_validate_user_parameters(args) {
    ...
    if ( ! req.user.company ) {
      ...
      start_date = moment(start_date, req.user.company.get_default_date_format()).format('YYYY-MM-DD');
      if (end_date) {
        end_date = moment(end_date, req.user.company.get_default_date_format()).format('YYYY-MM-DD');
      }
    }
