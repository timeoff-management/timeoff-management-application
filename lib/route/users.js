...
router.post('/add/', function(req, res){
  ...
  .then(new_user => Slack.promise_add_new_user_slack({
    company: current_company,
    admin_user: req.user,
    new_user: new_user
  }))
...
router.post('/import/', upload.single('users_import'), function (req, res) {
  let
...
router.post('/import-sample/', function(req, res){
  ...
  req.user
    ...
    .then(company => {
      ...
      content.unshift( ['email','slack_username','lastname', 'name', 'department'] );
...
router.get('/edit/:user_id/absences/', function(req, res){
  ...
  .then(args => {
    let
      ...
      remaining_allowance = allowance_obj.number_of_days_available_in_allowance,
      employee = args[1],
...
router.all('/search/', function(req, res){
  ...
  if ( ! req.accepts('json')) {
    ...
    var email = validator.trim(req.body.email || req.query.email).toLowerCase();
...
router.get('/', function(req, res) {
    ...
    if (typeof department_id === 'number' || (department_id && validator.isNumeric(department_id))) {
...
function users_list_as_csv(args) {
  ...
  let content = [['email', 'slack_username', 'lastname', 'name', 'department', 'remaining allowance', 'days used']];
...
function get_and_validate_user_parameters(args) {
    ...
    // Get user parameters
    var name = params.name && validator.trim(params.name),
        lastname = params.lastname && validator.trim(params.lastname),
        slack_username = params.slack_username && validator.trim(params.slack_username),
        email = params.email_address && validator.trim(params.email_address),
        department_id = params.department && validator.trim(params.department),
        start_date = params.start_date && validator.trim(params.start_date),
        end_date = params.end_date && validator.trim(params.end_date),
        adjustment = params.adjustment && validator.trim(params.adjustment),
        password = params.password_one && validator.trim(params.password_one),
        password_confirm = params.password_confirm && validator.trim(params.password_confirm),
        admin = (params.admin && validator.toBoolean(params.admin)) || false,
        auto_approve = (params.auto_approve && validator.toBoolean(params.auto_approve)) || false;
