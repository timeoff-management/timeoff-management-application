...
router.get('/edit/:user_id/absences/', function(req, res){
  ...
  .then( employee => employee.reload_with_leave_details({holiday_start_month:holidayMonthStart}))
...
router.get('/edit/:user_id/calendar/', async (req, res) => {
  ...
  try {
    ...
    calendar = await employee.promise_calendar({
      year: year.clone(),
     holiday_start_month: company.holiday_year_start_month,
      show_full_year: true,
    });
    companyEnriched = await employee.get_company_with_all_leave_types();
    employee = await employee.reload_with_leave_details({holiday_start_month: company.holiday_year_start_month, year });
    supervisors = await employee.promise_supervisors();
    userAllowance = await employee.promise_allowance({holiday_start_month: company.holiday_year_start_month, year });
