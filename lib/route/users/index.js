        ...
        employee
          ...
          .then(function(grouped_leaves){
            ...
            res.render('user_details', {
              ...
              leave_type_statistics : employee.get_leave_statistics_by_types({holiday_year_start_month:employee.company.holiday_year_start_month}),
...
router.get('/edit/:user_id/calendar/', async (req, res) => {
  ...
  let employee, calendar, companyEnriched, user, supervisors, userAllowance,holiday_year_start_month;
  ...
  res.render('user_details', {
    ...
    previous_year  :  moment.utc(year).month() > holiday_year_start_month ? moment.utc(year).format('YYYY'):moment.utc(year).add(-1,'year').format('YYYY'),
    current_year   : moment.utc(year).month() > holiday_year_start_month ? moment.utc(year).add(1,'year').format('YYYY'):moment.utc(year).format('YYYY'),
    next_year      : moment.utc(year).month() > holiday_year_start_month ? moment.utc(year).add(2,'year').format('YYYY'):moment.utc(year).format('YYYY'),
    show_full_year : true,
            ...
            // taken from allowance
            {
              ...
              where : {
                ...
                $or : {
                  date_start : {
                    $between : [
                      moment.utc(year).month(startingMonth ).startOf('month').format('YYYY-MM-DD'),
                      moment.utc(year).add(1, "year").month(startingMonth-1).endOf('month').format('YYYY-MM-DD HH:mm'), ]
                  },
                  date_end : {
                    $between : [
                      moment.utc(year).month(startingMonth ).startOf('month').format('YYYY-MM-DD'),
