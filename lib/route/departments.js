...
function get_and_validate_department(args) {
  ...
  // Get user parameters
  let name = validator.trim(req.body[no_suffix ? "name" : "name__" + index]),
    allowance = validator.trim(
      req.body[no_suffix ? "allowance" : "allowance__" + index]
    ),
    boss_id = validator.trim(
      req.body[no_suffix ? "boss_id" : "boss_id__" + index]
    ),
    include_public_holidays = validator.toBoolean(
      req.body[
        no_suffix
          ? "include_public_holidays"
          : "include_public_holidays__" + index
      ]
    ),
    is_accrued_allowance = validator.toBoolean(
      req.body[
        no_suffix ? "is_accrued_allowance" : "is_accrued_allowance__" + index
      ]
    );
  ...
  if (
    ...
    session.flash_error("New boss for " + department_name + " is unknown");
  }
...
function promise_to_extract_company_and_department(
  department_id,
  params,
  session,
  req,
...
router.get("/departments/edit/:department_id/", function(req, res) {
  ...
  Promise.try(function() {
    return promise_to_extract_company_and_department(
      department_id,
      req.query,
      req.session,
      req
