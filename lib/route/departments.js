...
function get_and_validate_department(args) {
  ...
  var
      ...
      include_public_holidays = params[no_suffix ? 'include_public_holidays' : 'include_public_holidays__' + index] && validator.toBoolean(
          params[no_suffix ? 'include_public_holidays' : 'include_public_holidays__' + index]
    );
...
function promise_to_extract_company_and_department(params, session) {
...
router.get('/departments/edit/:department_id/', function(req, res){
  ...
  Promise.try(function(){
      return promise_to_extract_company_and_department(req.query, req.session);
  })
...
router.post('/departments/edit/:department_id/', function(req, res){
  ...
  Promise.try(function(){
      return promise_to_extract_company_and_department(req.body, req.session);
  })
  .then(function(result){
    company    = result.company;
  ...
  .then(function(){
      if (params.remove_supervisor_id) {
      ...
      } else if (params.do_add_supervisors) {
