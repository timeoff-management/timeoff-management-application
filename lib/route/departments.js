...
function get_and_validate_department(args) {
  ...
  // Get user parameters
  let name = validator.trim(req.param(no_suffix ? "name" : "name__" + index)),
    allowance = validator.trim(
      req.param(no_suffix ? "allowance" : "allowance__" + index)
    ),
    boss_id = validator.trim(
      req.param(no_suffix ? "boss_id" : "boss_id__" + index)
    ),
