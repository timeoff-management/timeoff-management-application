...
function get_and_validate_department(args) {
    ...
    // Get user parameters
    var
        name = validator.trim(params[(no_suffix ? 'name' : 'name__' + index)]),
        allowance = validator.trim(params[(no_suffix ? 'allowance' : 'allowance__' + index)]),
        boss_id = validator.trim(params[(no_suffix ? 'boss_id' : 'boss_id__' + index)]),
        include_public_holidays = (params[no_suffix ? 'include_public_holidays' : 'include_public_holidays__' + index] && validator.toBoolean(
            params[no_suffix ? 'include_public_holidays' : 'include_public_holidays__' + index])) || false;
    ...
    // current company
    if (typeof boss_id !== 'number' && (!boss_id || !validator.isNumeric(boss_id))) {
        session.flash_error(
...
router.post('/departments/delete/:department_id/', function (req, res) {
    ...
    if (typeof department_id !== 'number' && (!department_id || !validator.isInt(department_id))) {
        console.error(
...
function promise_to_extract_company_and_department(department_id, params, session, req) {
    var company;
...
router.get('/departments/edit/:department_id/', function (req, res) {
    ...
    Promise.try(function () {
        return promise_to_extract_company_and_department(department_id, req.query, req.session, req);
    })
...
router.post('/departments/edit/:department_id/', function (req, res) {
    ...
    Promise.try(function () {
        return promise_to_extract_company_and_department(department_id, req.body, req.session, req);
    })
        .then(function (result) {
        ...
        .then(function () {
            if (req.body.remove_supervisor_id) {
                return promise_to_remove_supervisor({
                    supervisor_id: params.remove_supervisor_id,
                    company: company,
                    department: department,
                })
                    .then(function () {
...
function promise_to_remove_supervisor(args) {
    ...
    if (company.users.map(function (u) {
        return String(u.id)
