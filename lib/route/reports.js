...
router.get('/allowancebytime/', (req, res) => {
    ...
    let start_date = req.query.start_date && validator.toDate(req.query.start_date)
        ? moment.utc(req.query.start_date)
        : req.user.company.get_today();

    let end_date = req.query.end_date && validator.toDate(req.query.end_date)
    ...
    var current_deparment_id = typeof req.query.department === 'number' || (req.query.department && validator.isNumeric(req.query.department))
    ...
    Promise.join(
        ...
        .catch(error => {
            console.error(
                ...
                ' tried to access /reports/allowancebytime page: ', error, error.stack
            );
