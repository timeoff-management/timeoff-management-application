  ...
  team_view_details.users_and_leaves.forEach((ul) => {
    content.push(
      [ul.user.email, ul.user.lastname, ul.user.name]
        ...
        .concat(
          ul.statistics.leave_type_break_down.pretty_version.map(
            (it) => it.stat
          )
        )
...
const sortersForLeavesReport = {
  ...
  startDate: (a, b) =>
    moment.utc(a.startDate).toDate().valueOf() -
    moment.utc(b.startDate).toDate().valueOf(),
  endDate: (a, b) =>
    moment.utc(a.endDate).toDate().valueOf() -
    moment.utc(b.endDate).toDate().valueOf(),
  status: (a, b) => sorter(a.status, b.status),
  createdAt: (a, b) =>
    moment.utc(a.createdAt).toDate().valueOf() -
    moment.utc(b.createdAt).toDate().valueOf(),
...
router.get("/leaves/", async (req, res) => {
  ...
  const { startDate, endDate, departmentId, leaveTypeId } =
