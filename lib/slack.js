...
Slack.prototype.get_send_slack = function(){
  ...
  if (! config.get("send_slacks") ) {
    ...
    var token = config.get('slack_settings').token;
    var web = new WebClient(token);

    var push_slack = function(data){
      web.chat.postMessage({ 
        username: 'TimeOff Bot',
        channel: data.channel, 
        icon_url: 'https://emoji.slack-edge.com/T02D11B4K/williamhill/1d6fb5f835d3aace.jpg',
        attachments: [
          {
            title: 'Notification from TimeOff Holiday System',
            text: data.text,
          }
        ]
      })
