...
Email.prototype.promise_leave_request_emails = function(args){
  ...
  return getCommentsForLeave({leave})
    .then(comments => bluebird.join(
      ...
      leave.get('user')
        .promise_supervisors()
        .map(supervisor => promise_email_to_supervisor(comments)(supervisor)),
      () => bluebird.resolve()
    ));
};
...
Email.prototype.promise_leave_request_cancel_emails  = function(args){
  ...
  const promise_email_to_supervisor = self.promise_rendered_email_template({
  ...
  const promise_email_to_requestor = self.promise_rendered_email_template({
