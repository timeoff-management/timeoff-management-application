      ...
      .then((text) => {
        ...
        return (
          handlebars
            ...
            .then((final_email) => bluebird.resolve({
              subject: subject_and_body[0],
              body: final_email
            }))
        )
...
Email.prototype.promise_registration_email = function(args) {
  ...
  return self
    ...
    .then((email_obj) => send_mail({
      ...
      html: email_obj.body
    }).then((send_result) => user.record_email_addressed_to_me(email_obj).then(() => bluebird.resolve(send_result))))
}
...
Email.prototype.promise_forgot_password_email = function(args) {
  ...
  return user
    .getCompany()
    .then((company) => self.promise_rendered_email_template({
